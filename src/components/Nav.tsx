...
const Nav = () => {
    ...
    return (
        <>
            <nav className={pathname === "/bienvenido" || pathname.startsWith("/auth") ? "hidden" : `${pathname.startsWith("/dashboard") ? "lg:hidden" : "" } h-[80px] lg:h-[65px] w-full z-[300] lg:absolute sticky top-0 flex place-items-center place-content-between bg-tertiary lg:bg-transparent`}>
            ...
            <div style={{ position: 'fixed', zIndex: 100}} className={pathname === "/bienvenido" ? "hidden" : pathname === "/auth/register" || pathname === "/auth/login" ? "hidden" : (isChecked ? "translate-y-[0%]" : "translate-y-[-100%]") + ` lg:hidden h-[200px] z-[100] w-2/3 max-w-[300px] fixed rounded-bl-[100%] flex place-content-center top-[79px] right-0 bg-tertiary transition-all duration-[.5s] ease-in-menu`}>
